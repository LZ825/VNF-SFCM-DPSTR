...
model = Sequential([
    layers.GRU(20, input_shape=(SEQ_LEN, 1)),
    #全连接层只有一个预测值
    layers.Dense(1)
])
...
y_true = scaler.inverse_transform(y_test[:200])
y_pred = scaler.inverse_transform(test_pred[:200])

# with open("resource_Bi-GRU.txt.txt", "w") as file:
#     # 将列表数据逐行写入文件
...
plt.title('The predicted value compared to the true value')
plt.legend(loc='upper left')  # 将图例放置在右上角
plt.show()
end_time = time.time()
print("LSTM-GRU运行时间：", end_time-start_time)